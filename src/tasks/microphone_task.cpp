...
q15_t spectral_density[(SAMPLE_SIZE / 2) + 1]; // Buffer to store magnitude squared results
...
void run_microphone_task()
{
    ...
    while (!stop_task)
    {
        ...
        // Compute magnitude squared of the FFT output
        arm_cmplx_mag_squared_q15(freq_domain_signal, spectral_density, (SAMPLE_SIZE / 2) + 1);

        // Reset bin sums for each iteration
        uint32_t frequency_bin_max[12] = {0};
        for (size_t bin_border_index = 0; bin_border_index < 12; bin_border_index++)
        {
            int bin_lower_boundary = freq_bin_boundaries[bin_border_index];
            int bin_upper_boundary = freq_bin_boundaries[bin_border_index + 1];
            uint16_t max_value = 0;

            for (size_t i = bin_lower_boundary; i < bin_upper_boundary; ++i)
            {
                if (spectral_density[i] > max_value)
                {
                    max_value = spectral_density[i];
                }
            }

            frequency_bin_max[bin_border_index] = max_value;
        }
        ...
        for (size_t bin_index = 0; bin_index < 12; ++bin_index)
        {
            ...
            uint32_t scaled_value = frequency_bin_max[bin_index] >> 10; // Scale down to match hue range (adjust scaling factor as needed)
